...
<html class="no-js" lang="en">
  <head>
	...
	<link rel="icon" type="image/png" sizes="192x192" href="images/embx-a-favicon-192.png">
		...
		<div class="small-12 large-4 columns" id="detailcolumn">
			<div class='row'>
				<div class="small-12 columns">
					...
					<form>
						<input type='checkbox' id='bringmarketsnapshot'  />
						<label for='bringmarketsnapshot'>Bring Market Snapshot</label>
					</form>
				</div>
				...
				<div id="detailcontent" class="small-12 columns">
					...
					<a href='#' id='cptyrfqinits' class='button expand'>Cpty RFQ Stats</a>
				</div>
			</div>
		</div>
		<div class="small-12 large-8 columns">
			<h5 id="pageheader">Please select a trading day from the left to see RFQs</h5>
...
$('#tradingday').change(function(){
	...
	*/
	var mktss = '';
	if ($("#bringmarketsnapshot").prop('checked')){
		mktss = '&bringmarketsnapshot=true';
	} 
	$.get('utils/embx_json.php?pf=rfqs'+mktss+'&tradingday='+$('#tradingday').val(), function(data){
...
$('#cptyrfqinits').click(function(){
	$('#pageheader').html("RFQ Initiation and Response Stats for Counterparties");
	$.get('utils/embx_ajax.php?pf=cptyrfqinits',function(data){
		$("#pagecontent").html(data);
		//$(document).foundation();
	});
});
...
$('body').on('click','.stats',function(){
	var theid = this.id;
	$.get('utils/embx_ajax.php?pf=resprate&cpty='+this.id,function(data){
		$('#'+theid+'stats').html(data);
		});
});</script>
